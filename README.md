...
- The File Manager remote panel clearly shows that SFTP browsing is still pending.
...
The remote side is intentionally still a placeholder in this phase. SFTP browsing, upload, download, transfer progress, and conflict handling remain planned v0.3.0 work.

#### Known Limitations

- Complex shell-specific command parsing is out of scope for v0.2.0.
- SFTP browsing, transfer queues, and remote monitoring are roadmap items, not current features.
...
- File Manager 的远程面板会明确显示 SFTP 浏览仍在后续阶段实现。
...
当前阶段远程侧仍然是占位状态。SFTP 浏览, 上传, 下载, 传输进度和冲突处理仍属于 v0.3.0 后续阶段。

#### 已知限制

- 复杂 shell 专属命令解析不属于 v0.2.0 范围。
- SFTP 浏览, 传输队列和远程监控是路线图项目, 不是当前功能。
