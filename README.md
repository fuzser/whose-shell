...
- The File Manager transfer queue shows upload/download direction, status, progress, source path, target path, conflict policy, and errors.
- Single-file uploads and downloads support skip, overwrite, and rename-as-copy conflict handling.
...
File Manager dock -> Right-click local file -> Upload
File Manager dock -> Right-click remote file -> Download
...
Use the local file panel right-click menu to upload a selected local file to the current remote directory. Use the remote file panel right-click menu to download a selected remote file to the current local directory.

Before upload or download starts, choose a conflict policy for an existing target: skip, overwrite, or rename as a copy. The transfer queue shows queued/running/completed/failed state, progress bytes, source path, target path, policy, and failure reason when available. Transfers run in worker threads so terminal input and output remain responsive.

#### Known Limitations

- Complex shell-specific command parsing is out of scope for v0.2.0.
- Directory transfer, transfer cancellation, retry controls, and remote monitoring are roadmap items, not current features.
...
- File Manager 的传输队列会显示上传/下载方向, 状态, 进度, 来源路径, 目标路径, 冲突策略和错误信息。
- 单文件上传和下载支持 skip, overwrite 和 rename-as-copy 冲突处理。
...
File Manager 面板 -> 右键本地文件 -> Upload
File Manager 面板 -> 右键远程文件 -> Download
Sessions 面板 -> 双击连接
...
可以在本地文件面板右键选中的本地文件并上传到当前远程目录。也可以在远程文件面板右键选中的远程文件并下载到当前本地目录。

上传或下载开始前需要选择目标已存在时的冲突策略：skip, overwrite 或 rename as copy。传输队列会显示 queued/running/completed/failed 状态, 进度字节数, 来源路径, 目标路径, 策略和可用的失败原因。传输运行在 worker 线程中, 不会阻塞终端输入输出。

#### 已知限制

- 复杂 shell 专属命令解析不属于 v0.2.0 范围。
- 目录传输, 传输取消, retry 控件和远程监控是路线图项目, 不是当前功能。
